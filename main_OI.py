...
def find_BH(lon1,lat1,t1,lon2,lat2,t2,s_spat,s_temp):
    nx1 = len(lon1)
    nt1 = len(t1)
    ...
    BH = ((np.repeat(lon2[np.newaxis],n1,0) - np.repeat(lon1_tmp[np.newaxis].T,n2,1))**2.0 + (np.repeat(lat2[np.newaxis],n1,0) - np.repeat(lat1_tmp[np.newaxis].T,n2,1))**2.0)/s_spat**2.0 + (np.repeat(t2[np.newaxis],n1,0) - np.repeat(t1_tmp[np.newaxis].T,n2,1))**2.0/s_temp**2.0
    BH = np.exp(-BH)

    return BH

def find_HBH(lon1,lat1,t1,lon2,lat2,t2,s_spat,s_temp):
    n1 = len(lon1)    
    n2 = len(lon2)

    HBH = ((np.repeat(lon2[np.newaxis],n1,0) - np.repeat(lon1[np.newaxis].T,n2,1))**2.0 + (np.repeat(lat2[np.newaxis],n1,0) - np.repeat(lat1[np.newaxis].T,n2,1))**2.0)/s_spat**2.0 + (np.repeat(t2[np.newaxis],n1,0) - np.repeat(t1[np.newaxis].T,n2,1))**2.0/s_temp**2.0
    HBH = np.exp(-HBH)
...
path_for_save = cwd + '/data/results/OI/'
...
for ix in tqdm(range(ssh_true.shape[1])):
    ...
    BH_tmp = find_BH(lon1[ix:ix+1]*0,lat1[ix:ix+1]*0, time_true[it_neighbor_tmp], lon_at_tmp*0,lat_at_tmp*0,time_at_tmp,s_spat,s_temp)
    HBH_tmp = find_HBH(lon_at_tmp*0,lat_at_tmp*0,time_at_tmp,lon_at_tmp*0,lat_at_tmp*0,time_at_tmp,s_spat,s_temp)

    rootvar1_tmp = np.sqrt(var_ssh[ix])*np.ones(365)
    cov1_tmp = H_tmp.dot(cov_ssh[ix])
    cov2_tmp = H_tmp.dot(cov_ssh).dot(H_tmp.T)
    BH_tmp = BH_tmp*cov1_tmp
    HBH_tmp = HBH_tmp*cov2_tmp		
